...
# # # # # # # # # # # # # # # # # # # # # # # #
def grossVal(Matrix,i,j,WEIGHTS) :
    # # print (WEIGHTS)
...
def greedyMove(Matrix,WEIGHTS) : 
    ...
    if len(empty_i) >= 1 :
        ...
        return chooseBest(Matrix,possible_next_i,possible_next_j,WEIGHTS)   
            # tmp[move_i][move_j] = 0
    # # print(possible_next_i,possible_next_j)
    
def nextMove(Matrix,player,policy,WEIGHTS) :
    if player == PLAYER_X and policy == EXPLORATERY :
        return randomMove(Matrix,player)
    if player == PLAYER_O :
        return randomMove(Matrix,player)
    if player == PLAYER_X and policy == GREEDY :
        return greedyMove(Matrix,WEIGHTS)

def gamePlay(Matrix,policy,WEIGHTS):
    old_Matrix = Matrix.copy()
    move_x_i, move_x_j = nextMove(Matrix,1,policy, WEIGHTS)
    # # print("gross value : ", grossVal(old_Matrix,move_x_i,move_x_j,WEIGHTS))
    ...
    # # print(Matrix)
    old_Matrix = Matrix.copy()
    move_o_i, move_o_j = nextMove(Matrix,-1,policy,WEIGHTS)
    ...
    # # print(Matrix)
    return gamePlay(Matrix,policy,WEIGHTS)
...
def games(weight) :
    ...
    for i in range(game_count) :
        ...
        game_result = gamePlay(Matrix,policy,WEIGHTS)
        # print(game_result)
        if game_result == WIN :
            # print("WIN")
            win_count += 1
    ...
    res = [win_percent,draw_percent,lost_percent]
    return res
