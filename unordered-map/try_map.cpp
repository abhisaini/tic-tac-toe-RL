...
void backUp(state &prevState,state &currState, float alpha) {
		string key = matToString(prevState.mat);
		stateMap[key] = stateMap[key] + alpha*(currState.val - stateMap[key]);
}
...
void pushBack(state S){
	...
	state S1(gridSize);
	equate(S, S1);
	stateMap.insert(make_pair(matToString(S1.mat), S1.val));
}
...
void playGame(float epsilon, float alpha, int gridSize){ // plays a game, objective is to update the stateArray and value function table
	...
	while(1){
		...
		/****** replacing starts here ***/

		if( policy == GREEDY ) backUp (dummyState , xState , alpha); /**ends here **/
