...
string plotFile(){
	string epsilon, alpha1,alpha2,gridSize,trains,checkCount;
	cout<<"What was  epsilon  ?"<<endl;
	std::cin >> epsilon;
	cout<<"What was  Alpha 1 ?"<<endl;
	std::cin >> alpha1;
	cout<<"Enter the Alpha 2 again :-)"<<endl;
	std::cin >> alpha2;
    ...
    string fileName =
    "Results/Grid-" + gridSize+ "/ Alph1-"+alpha1+"_Alph2-" +alpha2 +"_Ep-" +epsilon + "_Trainings-"
...
void plotGraph(std::vector<float> Trainings,std::vector<float> winPercent1,std::vector<float> lostPercent1,std::vector<float> winPercent2,std::vector<float> lostPercent2,string plotfileName){
	using namespace plt;
	named_plot("Alp-1 Win",Trainings,winPercent1,"b--");
	named_plot("Alp-2 Win",Trainings,winPercent2,"g");
	named_plot( "Alp-1 Lost",Trainings,lostPercent1 ,"r--");
	named_plot("Alp-2 Lost",Trainings,lostPercent2,"pink");
	legend();
	ylim(-5, 105);
	title("Percent vs Trains \n Comparing Probablities for Alpha");
	save(plotfileName);
...
int main(int argc, char **argv){
	...
	float epsilon, alpha1,alpha2;
	int gridSize,trains;
	float checkCount;

	cout<<"What Epsilon u want,choose any number between 0 to 1"<<endl;
	std::cin >> epsilon;
	cout<<"What 1st alpha to compare(Enter the higher alpha first),choose any number between 0 to 1 : "<<endl;
	std::cin >> alpha1;
    cout<<"What 2nd alpha to compare,choose any number between 0 to 1 : "<<endl;
	std::cin >> alpha2;
    ...
    string plotfileName = "Plot-CompAlph-" + fileName + ".png";
    string textfileName = "Text-CompAlph-" + fileName + ".txt";
	...
	for (int i = 0; i < trains; i++) {
		playGame(epsilon, alpha1, gridSize);
        float gamesWin = 0;
      	float gamesDraw = 0;
      	float gamesLose = 0;
		...
		cout<<"Alph1 : "<<i<<"th training :=  Win Percent : "<<winPercent1[i]<<" | Lost Percent :"<<lostPercent1[i]<<endl;
		fout<<"Alph1 : "<<i<<"th training :=  Win Percent :"<<winPercent1[i]<<" | Lost Percent :"<<lostPercent1[i]<<endl;
	}
    ...
    for (int i = 0; i < trains; i++) {
        playGame(epsilon, alpha2, gridSize);
        float gamesWin = 0;
        float gamesDraw = 0;
        float gamesLose = 0;
        for (int j = 0; j < int(checkCount); j++) {
        ...
        cout<<"Alph2 : "<<i<<" th training :=  Win Percent : "<<winPercent2[i]<<" | Lost Percent :"<<lostPercent2[i]<<endl;
        fout<<"Alph2 : "<<i<<" th training :=  Win Percent :"<<winPercent2[i]<<" | Lost Percent :"<<lostPercent2[i]<<endl;
    }
